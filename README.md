...
Loopforge 是一套面向 AI 编程助手（如 Trae IDE）的 **Skill 与 Rules 配置体系**。它定义了名为 **Loop Engineering** 的迭代自审流程：让 AI 对代码进行 6 轮有序的审查—修复—验证循环，每轮聚焦一个质量维度，配合 5 个联动 Skill 协同工作，逐步消除逻辑错误、状态问题、交互缺陷和性能隐患。

**解决的问题**：AI 改代码时容易"凭感觉一把梭"——方向发散、遗漏边界、修复不验证。Loopforge 把代码质量加固变成一个有章可循的工程流程。
...
loop/
├── rules/
│   └── user_rules.md              # 用户级规则（编码规范 + Loop Engineering 流程规则）
└── skills/
    ├── loop-engineering/
...
[user_rules.md](rules/user_rules.md) 包含两层规则：

1. **基础编码规范** — 命名、重构原则（基于 Martin Fowler《重构》）、性能优化
2. **Loop Engineering 流程规则** — 强制约束、联动 skill 调用顺序、反例与失败案例

## 快速开始

1. 将 `loop/` 目录下的 `rules/` 和 `skills/` 配置到你的 AI 编程助手（如 Trae IDE）中
...
[https://github.com/kipozannaki/Loopforge](https://github.com/kipozannaki/Loopforge)
